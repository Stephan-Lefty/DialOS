...
| **"Satz löschen"** (delete sentence) *(dictation only)* | Removes what was recognised last - everything since the previous speech pause - and says what is gone: "Gestrichen: Die Rechnung liegt im Schrank bei." Since 2026-09-15 (Stephan asked how to handle slips of the tongue). **Only valid with half a second of silence before and after** - checking against Piper, ordinary letter text otherwise produced a complete "satz löschen" ("Den ersten Satz habe ich gelöscht") or "satz wiederholen". While Anna answers, dictation does not listen. |
| **"Satz wiederholen"** (repeat sentence) *(dictation only)* | Reads out what was recognised last **without** deleting anything: "Zuletzt: …". That way a blind user notices a mishearing before dictating on. Same safeguards as "Satz löschen". |
